...
export default class VideoScreenshotStream extends ImageScreenshotStream {
  ...
  findExecutable() {
    ...
    return Promise.all([this.findExecutableUsingOptions(), this.findExecutableUsingEnvironment(), this.findExecutableUsingWhich()]).then(values => {
      this._cachedExecutable = values.find(item => {
        return item;
      });
      return Promise.resolve(this._cachedExecutable);
    });
